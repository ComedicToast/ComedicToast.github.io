---
title: "Lab 8 Blog Post"
description: "Web Scraping (and cheese)"
author:
  - name: ComedicToast
...
```{r}
# Helper function to extract text content from a specified CSS selector on a webpage
get_cheese_attribute <- function(webpage, css) {
  result <- webpage %>%
    html_elements(paste0(".", css, " p")) %>%
    html_text(trim = TRUE)

  # Return an empty string if nothing is found, otherwise return the first result
...
scrape_cheese <- function(cheese, base_url, delay = 1) {
  ...
  # Include system delay so we don't overload the website
  Sys.sleep(delay)

  # tryCatch handles errors nicely, providing a structured way to manage them
  # and still return a blank data frame as output in case of an error.
  tryCatch({
    # Read the webpage URL
    webpage <- read_html(page_url)

    # Grab and store every desired attribute using the external helper function
    cheese_taste <- get_cheese_attribute(webpage, "summary_taste")
    cheese_milk <- get_cheese_attribute(webpage, "summary_milk")
    cheese_type <- get_cheese_attribute(webpage, "summary_moisture_and_type")
    cheese_family <- get_cheese_attribute(webpage, "summary_family")
    cheese_country <- get_cheese_attribute(webpage, "summary_country")

    # Return the data frame with all scraped attributes, with some additional string cutting
    data.frame(
      Name = cheese,
    ...
    # Give a warning message if an error occurs
    warning(paste("Failed to scrape cheese:", cheese, "-", error_message$message))

    # Return an empty data frame with NA values in case of an error
    data.frame(
      Name = cheese,
      Taste = NA_character_,
      Milk = NA_character_,
...
# Directly apply scrape_cheese to the cheese_list using map_dfr
# The .x refers to each element in cheese_list as map_dfr iterates
cheese_output <- map_dfr(cheese_list, ~scrape_cheese(.x, base_url, delay = 1))
...
I used mapping to apply the `scrape_cheese()` function to a vector of different cheeses. The substring method I used (sub()) might be able to be made into a helper function since it is repeated in a couple of places. The iteration I do have present seems efficient since it directly applies the existing function to a vector and outputs a data frame.