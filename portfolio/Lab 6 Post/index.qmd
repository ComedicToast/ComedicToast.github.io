...
age_calc <- function(birthdates) {
  ...
  #get birthdays, today's date, divide by 365.25 to account for leap years
  birthdates <- as.Date(birthdates)
  today <- Sys.Date()
  ages <- time_length(interval(birthdates, today), "years") 
  return(ages)
}
...
both_na <- function(V1, V2){
  ...
  #check number of positions in both that are not NA
...
#checks to make sure all the supplied columns are numeric
  ...
  #filter by the arrival and delay and departure times
  filtered_df <- df %>%
...
#filter by number of hours input, similar to earlier function otherwise  
  filtered_df <- df %>%
...
summarize_weather <- function(df, variable){
  #grab vector from the data frame
  non_num <- df %>%
    pull( {{ variable }}) %>%
    is.numeric()
  #make sure that mf is numeric
   if (!non_num) {
    stop(
...
standardize_time <- function(df, clock_time){
  ...
  non_num <- df %>%
    pull({{ clock_time }}) %>%
    is.numeric()
  #make sure that mf numeric as hell
   if (!non_num) {
    stop(
      str_c("You must use a numeric variable for the `clock_time` input!")
      )
  }
  #change the variable to read as clock time
  df %>%
    mutate(
...
sorted_bars <- function(df, var){
  ...
  #if too many categories then flip the graph so that it read better
  if (total_vals > 10) {
    p <- p + coord_flip()
  }
...
#grab names of char vars
char_vars <- mpg %>%
  select(where(is.character)) %>%
  names()
#for the char vars, apply the function
